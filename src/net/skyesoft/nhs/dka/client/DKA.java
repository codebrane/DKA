 ...
 */
public class DKA implements EntryPoint, HistoryListener {
  ...
  protected void loadSinks() {
    list.addLogo();
    list.addSink(Audit.init());
    list.addSink(IVFluids.init());
    list.addSink(TreatmentAndInvestigation.init());
    list.addSink(RoutineBloods.init());
    list.addSink(TransferToSCInsulin.init());
    list.addSink(EducationAndFollowUp.init());
    list.addSink(About.init());
    list.addCurrentStudyNoDisplay();
    list.addPreviousDataLoader();
    list.addSaveButton();
  }

  private void showInfo() {
    show(list.find("General"), false);
  }
}
